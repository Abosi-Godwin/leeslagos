...
const BestSellers = () => {
  ...
  return (
    <div className="py-5 my-10 px-4 w-full bg-gray-100 rounded-md">
      <h1 className="font-heading text-2xl font-bold text-center py-6 mb-6">Best Sellers</h1>
      <Swiper
        ...
        {splitted.map((data, index) => {
          return (
            <SwiperSlide key={index}>
              <h1 className="font-bold pb-4 text-xl">{tagLines[index]}</h1>
                  ...
                  >
                    ...
                    <div className="">
                      ...
                      <div className="flex gap-5">
                        <p className="text-sm tracking-tight">{formatCurrency(item.price)}</p>
                        <p
                          className="text-sm
                                            tracking-tight line-through "
                        >
                          {formatCurrency((item.price / 90) * 100)}
                        </p>
                      </div>
                    </div>
                  </div>
